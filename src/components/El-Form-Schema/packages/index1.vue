...
export default {
  ...
  computed: {
    isCanExpand() {
      return (
        this.expandNumber > 0 &&
        Object.keys(this.schema).length > this.expandNumber
      );
    },
  },
  watch: {
  ...
  updated() {
    ...
    Array.from(document.querySelectorAll(".el-form-item-inline")).forEach(
      (el) => {
        ...
        if (el.querySelector(".is-required") && slotLabelWidth) {
          el.querySelector(".el-form-item__content").style.marginLeft =
            slotLabelWidth;
    ...
    Array.from(document.querySelectorAll(".is-set-inline")).forEach((el) => {
      ...
      if (
        el.querySelector(".el-form-item") &&
        !el.querySelector(".el-form-item").style.display
      ) {
  ...
  methods: {
    ...
    setExpTpl(component) {
      ...
      if (
        typeof component.slot === "string" &&
        /\$\{.+?\}/g.test(component.slot)
      ) {
        component["slot_template_string"] = component.slot;
      } else if (
        typeof component.slot === "object" &&
        /\$\{.+?\}/g.test(component.slot.after)
      ) {
    ...
    setExp(component) {
      ...
      if (
        ...
        component.rules &&
        typeof component.rules.required === "string"
      ) {
    ...
    initComponentDefaultProps(component, watchConfigExp) {
      ...
      component.isInput = component.hasOwnProperty("isInput")
        ? component.isInput
        : true;
      ...
      component.rules =
        component.rules ||
        (component.required ? { required: true, message: "必填" } : null);
      // 监听config表达式的情况
      ...
      switch (component.tag) {
        ...
        case "el-input":
          component.attrs.placeholder =
            ...
            `请输入${
              /\$\{.+?\}/g.test(component.label) ? "" : component.label
            }`;
          break;
        case "el-date-picker":
          ...
          if (component.props.type === "daterange") {
            ...
            component.props.valueFormat =
              component.props.valueFormat || "yyyy-MM-dd";
          } else if (component.props.type === "datetimerange") {
            component.default = component.default || [];
            component.props.valueFormat =
              component.props.valueFormat || "yyyy-MM-dd HH:mm:ss";
            component.props.defaultTime = component.props.defaultTime || [
              "00:00:00",
              "23:59:59",
            ];
      ...
      switch (schema.tag) {
        case "object":
          if (schema.components) {
            ...
            if (schema.type === "card" && !schema.hasOwnProperty("border"))
              schema.border = true;
            for (const _key in schema.components) {
              ...
              Object.keys(schema.slot).forEach((key) => {
                ...
                if (obj instanceof Object && obj["vmodel"]) {
                  ...
                  if (key === "after") {
                    ...
                    schema.slot[key].style.marginBottom =
    ...
    getValidateProps(field) {
      ...
      if (typeof val !== "object" && !Array.isArray(val)) {
        ...
        return val.length === 0
          ? [field]
          : val
              .map((item, index) => this.getValidateProps(`${field}[${index}]`))
              .flat(Infinity);
      } else {
        if (val) {
              ...
              if (typeof val[next] !== "object" && !Array.isArray(val[next])) {
                ...
                val[next].length === 0
                  ...
                  : val[next]
                      .map((item, index) =>
                        prev.push(
                          ...this.getValidateProps(`${field}.${next}[${index}]`)
                        )
                      )
                      .flat(Infinity);
              } else {
    ...
    validateField(props, cb) {
      ...
      return (
        ...
        this.$refs[this.refName].validateField(props, cb)
      );
    },
    /**
     * @description: 部分表单校验
    ...
    validateFieldPromise(props) {
      ...
      return Promise.all(
        props.map(
          (item) =>
            new Promise((resolve) => {
              this.$refs[this.refName].validateField(item, (valid) =>
                resolve(valid)
              );
            })
        )
    ...
    clearValidate(props = []) {
      ...
      return (
        this.$refs[this.refName] &&
        this.$refs[this.refName].clearValidate(props)
      );
    },
    /**
    ...
    onEnterSearch() {
      if (this.isSearchForm && this.useEnterSearch) {
        document.onkeyup = (event) => {
          ...
          if (e && e.keyCode === 13) {
            if (
              !isEqual(this.formValues, this.enterFormValues) &&
              this.$refs[this.refName]
            ) {
              this.enterFormValues = JSON.parse(
      ...
      ? h(
          "div",
          {
            style: {
              ...
              borderShadow: "0 2px 12px 0 rgba(0,0,0,.1)",
            },
          },
          [FormRender]
        )
